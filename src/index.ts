...
import { embeddingPipeline, formatEmbeddingModelId, getConfiguredEmbeddingModelId } from './core/embedding-pipeline';
...
class ZotSeekPlugin {
  ...
  private initDefaultPreferences(): void {
    ...
    // Embeddings are generated by the configured online provider/model
    const defaults: { [key: string]: any } = {
      ...
      'zotseek.embeddingProvider': 'voyage',
      'zotseek.embeddingModel': 'voyage-3.5-lite',
      'zotseek.embeddingApiKey': '',
      // Chunking defaults for remote embedding requests
  ...
  private async ensureIndexModelCompatibility(actionLabel: string): Promise<void> {
    await this.ensureStoreReady();
    if (!this.vectorStore) return;

    const stats = await this.vectorStore.getStats();
    if (stats.indexedPapers === 0 || stats.modelId === 'none') {
      return;
    }

    const currentModelId = getConfiguredEmbeddingModelId();
    if (stats.modelId !== currentModelId) {
      throw new Error(
        `Your existing index uses ${formatEmbeddingModelId(stats.modelId)}, but current settings use ${formatEmbeddingModelId(currentModelId)}. Rebuild the index before ${actionLabel}.`
      );
    }
  }
  ...
  public async getStats(): Promise<{
    ...
    try {
      ...
      return {
        ...
        modelId: formatEmbeddingModelId(stats.modelId),
        rawModelId: stats.modelId,
        storageSize,
        databasePath,
        lastIndexed,
        lastIndexDuration,
  ...
  private async indexItems(items: any[]): Promise<void> {
    ...
    try {
      ...
      await this.ensureIndexModelCompatibility('adding new items');

      // Reset pipeline to ensure fresh initialization
      embeddingPipeline.reset();

      progressWindow.updateProgress('Preparing embedding provider...', null);
      await embeddingPipeline.init();
      this.logger.info('Embedding pipeline initialized')
      progressWindow.addLine('✓ Embedding provider ready', 'chrome://zotero/skin/tick.png');
      ...
      for (let batchStart = 0; batchStart < itemsToIndex.length; batchStart += CHECKPOINT_BATCH_SIZE) {
        ...
        const embeddingMap = await embeddingPipeline.embedBatch(batchChunks, (progress) => {
          progressWindow.updateProgressWithETA(
            `Batch ${batchNumber}/${totalBatches}: Embedding chunks`,
            batchStart + Math.floor((progress.current / Math.max(batchChunks.length, 1)) * batchItems.length),
            itemsToIndex.length
          );
        });
        ...
        const failedChunks = batchChunks.length - embeddingMap.size;
  ...
  private async indexItemsSilent(items: any[]): Promise<void> {
    ...
    try {
      ...
      // Reset pipeline to ensure fresh initialization
      await this.ensureIndexModelCompatibility('auto-indexing new items');

      itemRow.setText('Preparing provider...');
      embeddingPipeline.reset();
      await embeddingPipeline.init();
      ...
      const embeddingMap = await embeddingPipeline.embedBatch(textsForEmbedding, (progress) => {
        itemRow.setText(`Embedding ${progress.current}/${textsForEmbedding.length}...`);
      });

      const failedChunks = textsForEmbedding.length - embeddingMap.size;
      const failedItems = new Set(
        textsForEmbedding
          .filter(item => !embeddingMap.has(item.id))
          .map(item => item.title)
      );
